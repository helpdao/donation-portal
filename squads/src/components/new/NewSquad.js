...
//FORM VALIDATION
    ...
    const checkName = () => {
        return new Promise((resolve, reject) => {
            findSquad({name:name}).then((response) =>{
                console.log("Valid Name?")
                console.log(response)
                if(response.data.squads.length === 0){
                    resolve();
                }else{
                    message.error('This name is in use, you need to choose a different Squad Name')
                    reject();
                }
            }).catch((err) => {
            ...
            if(formValid !== false && walletConnected !== false){
              ...
              createSquad(data).then((response) => {
                if (response.status === 200) {
                    ...
                    console.log(err)
                    message.error("Oops, something goes wrong, please review all the fields and try it again in a while.")
                });                
            }
        }
      }
    ...
    },
    {
        ...
        content:(
            <div>
                ...
                <Input name="name" placeholder="Name" value={name} onChange={(evt) => setName(evt.target.value)}/>
                <label>Squad Invitation Link</label>
                <Input name="inviteLink" placeholder="InviteLink" value={inviteLink} onChange={(evt) => setInviteLink(evt.target.value)}/>
                <label>Squad Description</label>
                <TextArea rows={20} name="description" placeholder="Description" value={description} onChange={(evt) => setDescription(evt.target.value)}/>
